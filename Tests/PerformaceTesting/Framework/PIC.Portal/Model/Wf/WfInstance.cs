...
namespace PIC.Portal.Model
{
	...
	public partial class WfInstance
    {
        ...
        public void DoDiscard()
        {
            ...
            {
                throw new MessageException("只有已启动的流程可以作废。");
            }

            // 当前任务编码为开始，且当前用户为申请人时才可以作废
            if (!StringHelper.IsEqualsIgnoreCase(this.FlowState.Current.TaskCode, "START")
                || !StringHelper.IsEqualsIgnoreCase(this.OwnerID, PortalService.CurrentUserInfo.UserID))
            {
                throw new MessageException("只有申请人在开始任务时才可以作废流程。");
            }
