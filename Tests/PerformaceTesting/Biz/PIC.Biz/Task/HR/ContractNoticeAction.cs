...
namespace PIC.Biz.Task.HR
{
    public class ContractNoticeAction : IAction
    {
        /// <summary>
        /// 合同到期提醒（一般由计划任务调用）
        /// </summary>
        public ActionResult Execute(Portal.Model.Task task)
        {
            ...
            try
            {
                ...
                {
                    Message.SysSendTo("SYS", "Sys_Adm", "合同到期提醒标题", "合同到期提醒内容", null, null);

                    result = new ActionResult(ActionResult.ResultEnum.Completed, "合同到期提醒任务执行完成，已发送合同提醒信息！");
