...
{
    [Serializable]
    public abstract class BizTreeNodeModelBase<T> : BizModelBase<T>, IEntity where T : BizTreeNodeModelBase<T>
    {
        #region 成员变量
        ...
        public virtual void CreateAsChild(string parentID, int position)
        {
                ...
                && (String.IsNullOrEmpty(target.Path) || target.Path.IndexOf(this.ID) < 0))
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    try
                    {
                        ...
                        this.RefreshStructureInfo(target.ID, position, conn);
        ...
        /// </summary>
        private void RefreshStructureInfo(string newParentID, int newPosition, IDbConnection conn)
        {
