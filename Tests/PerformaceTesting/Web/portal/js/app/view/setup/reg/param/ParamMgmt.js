Ext.define('PIC.view.setup.reg.param.ParamMgmt', {
    extend: 'PIC.Page',
    alternateClassName: 'PIC.RegParamMgmtPage',

    requires: [
        'PIC.model.sys.reg.ParameterCatalog',
    ],
    ...
    constructor: function (config) {
        ...
        me.dataStore = new Ext.create('PIC.TreeGridStore', {
            model: 'PIC.model.sys.reg.ParameterCatalog',
            dsname: 'EntList',
        ...
        me.mainPanel = Ext.create("PIC.TreeGridPanel", {
            ...
            minSize: 200,
            maxSize: 500,
            formparams: { url: "ParamCatalogEdit.aspx", style: { width: 600, height: 260} },
    ...
    onFormSubmit: function (args) {
        ...
        var id = args.params.frmdata.ParameterID,
            action = args.action;
        ...
        new_rec = Ext.create('PIC.model.sys.reg.ParameterCatalog', args.respdata.frmdata);
    ...
    onItemActived: function (rec, mode, params) {
        ...
        if (rec) {
            if (!frameContent.PICPage) {
                frameContent.location.href = "ParamList.aspx?cid=" + rec.getId();
            } else if (frameContent.PICPage.reloadPage) {
                frameContent.PICPage.reloadPage(rec.getId(), rec.getData());   // 异步加载
            }
        }
    }
});