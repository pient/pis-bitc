    ...
    constructor: function (config) {
        ...
        if (me.isFlowStarted()) {
            ...
            if (me.aid && !me.isFlowEnd() && !me.isFlowReadonly()) {
                flowInfoItems.push(me.approveForm);
            }
        ...
        var displayDelete = (me.iid && ("update".equals(pgAction)) && me.instanceData && ("New".equals(me.instanceData.Status) || "Draft".equals(me.instanceData.Status)));
        var displaySave = displayDelete || ((me.did || me.aid) && !me.isFlowReadonly() && !me.isFlowEnd());
        var displaySubmit = displaySave;
        var displayDiscard = me.iid && me.stage.equals("start");    // 被驳回到开始节点时，申请人可以作废此流程
        ...
        me.mainPanel = Ext.create("PIC.ExtPanel", {
            ...
            tbar: [{
                ...
                text: '确定',
                hidden: !displaySubmit,
                handler: function () {
                    me.doSubmit();
                }
            }, '-', {
                text: '删除',
                hidden: !displayDelete,
                ...
                text: '作废',
                hidden: !displayDiscard,
                xtype: 'picdeletebutton',
                handler: function () {
                    me.doDiscard();
                }
            }, '-', docExpMenu, { xtype: 'picclosebutton' }, '->',
            '-', {
                xtype: 'pichelpbutton',
                handler: function () {
                    me.onHelp();
