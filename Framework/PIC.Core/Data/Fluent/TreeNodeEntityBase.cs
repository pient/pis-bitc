...
namespace PIC.Data.Fluent
{
    ...
    /// 树形属性节点实体
    /// </summary>
    public abstract class TreeNodeEntityBase<T> : ITreeNodeEntity<T> where T : ITreeNodeEntity<T>
    {
        #region 成员变量
        ...
        public virtual string ID
        {
            get
            {
                return ObjectHelper.ConvertValue<string>(this.GetPrimaryValue());
            }
        }
        ...
        public static string GenerateNewID()
        {
            ...
            // return (string)idGenerator.Generate();
        ...
        public static IList<T> FilterChildNodes(IEnumerable<T> nodes)
        {
            ...
            {
                // 节点路径中不包含任何节点的Id
                if (String.IsNullOrEmpty(tnode.Path) || nodes.Count(tent => tnode.Path.Contains(ObjectHelper.ConvertValue<string>(tent.ID))) <= 0)
                {
                    rtnNodes.Add(tnode);
                }
            }
