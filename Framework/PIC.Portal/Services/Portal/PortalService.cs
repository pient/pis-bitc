...
namespace PIC.Portal
{
    ...
    public class PortalService
    {
        ...
        public const string TEST_USER_NAME_KEY = "test_user_name";  // 测试用户key
        ...
        public const string PORTAL_RUNNING_MODE_KEY = "RunningMode";
        ...
        public static RunningModeEnum RunningMode
        {
            get
            {
                RunningModeEnum runningMode = RunningModeEnum.Normal;
                if (PICConfigurationManager.AppSettings.ContainsKey(PORTAL_RUNNING_MODE_KEY))
                {
                    runningMode = CLRHelper.GetEnum<RunningModeEnum>(
                        PICConfigurationManager.AppSettings[PORTAL_RUNNING_MODE_KEY],
                        RunningModeEnum.Normal, true);
                }
        ...
        public static OperationRequest NewOperationRequest()
        {
            ...
            if (RunningMode == RunningModeEnum.PerformanceTesting)
            {
                if (CurrentUserInfo != null)
                {
                    op.Params.Set(PortalService.TEST_USER_NAME_KEY, CurrentUserInfo.LoginName);
                }
            }
        ...
        {
            return PortalService.Provider.GetOnlineUserCount();
        }
