...
{
    [Serializable]
    public abstract class DocTreeNodeModelBase<T> : DocModelBase<T>, IEntity where T : DocTreeNodeModelBase<T>
    {
        #region 成员变量
        ...
        public virtual Guid? ID
        {
            get
            {
                return this.GetPrimaryValue().ToGuid();
            }
        }
        ...
        public virtual T Parent
        {
            get
            {
                if (!this.ParentID.IsNullOrEmpty())
                {
                    this.RefreshParent();
                }
        ...
        {
            ICriterion crit = Expression.Like(Prop_Path, this.ID.ToString(), MatchMode.Anywhere);
        ...
        public virtual void CreateAsSibling(object siblingID)
        {
        ...
        public virtual void CreateAsChild(object parentID, int position)
        {
            CreateAsChild(EntityBase<T>.Find(parentID), position);
        }
        ...
        public virtual void CreateAsChild(T parent, int position)
        {
            this.InsertAt(parent, position);
        }
        ...
        /// </summary>
        /// <param name="module"></param>
        public virtual void CreateAsChild(object parentID)
        {
        ...
        public virtual void InsertAt(T parent, int position)
        {
            this.ParentID = parent.ID;
            this.Path = CombinePath(parent.Path, parent.ID.ToString());
        ...
        public virtual void MoveAsChild(object targetID)
        {
        ...
        /// 根据ID移动为目标节点的兄弟节点
        /// </summary>
        public virtual void MoveAsSibling(object targetID)
        {
        ...
        /// 根据ID复制为目标节点子节点
        /// </summary>
        public virtual void CopyAsChild(object targetID)
        {
        ...
        public virtual void CopyAsChild(T target)
        {
            ...
            try
            {
                if (conn != null)
                {
                    ...
                    if (target.ParentID.IsNullOrEmpty())
                    {
        ...
        /// 根据ID复制为目标节点兄弟节点
        /// </summary>
        public virtual void CopyAsSibling(object targetID)
        {
        ...
        private DataTable GetCopyTable(T target, PasteAsEnum patype)
        {
            ...
            else if (patype == PasteAsEnum.Child)
            {
                ...
                adjustPath = CombinePath(target.Path, target.ID.ToString());
            }
        ...
        protected virtual void ChangeParent(object targetID, int position)
        {
        ...
        protected virtual void ChangeParent(T target, int position)
        {
            ...
            ChangeParent(target, position, session.Connection);
        }
        ...
        /// <param name="target"></param>
        /// <param name="position"></param>
        protected virtual void ChangeParent(T target, int position, IDbConnection conn)
        {
            // 目标节点必须存在且与当前父节点不同,当前节点不能为目标节点下层节点
            if (target != null && this.ParentID != target.ID && this.ID != target.ID
                && (String.IsNullOrEmpty(target.Path) || target.Path.IndexOf(this.ID.ToString()) < 0))
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    try
                    {
                        var orgParentID = this.ParentID;
                        int orgSortIndex = this.SortIndex.GetValueOrDefault();

                        this.RefreshStructureInfo(target.ID.Value, position, conn);

                        string sql = String.Empty;

                        if (!orgParentID.IsNullOrEmpty())
                        {
                            // 调整源父节点的IsLeaf状态
        ...
        /// </summary>
        private void RefreshStructureInfo(Guid newParentID, int newPosition, IDbConnection conn)
        {
        ...
        protected virtual void ExchangeSiblingPosition(object sibling1ID, object sibling2ID)
        {
        ...
        {
            return this.ParentID.IsNullOrEmpty() || PathLevel == RootNodeLevel;
        }
