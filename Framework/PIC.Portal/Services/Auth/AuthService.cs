...
namespace PIC.Portal
{
    public class AuthService
    {
        ...
        // 初始化化用户上下文列表
        protected Hashtable userContextList = Hashtable.Synchronized(new Hashtable());

        /// <summary>
        /// 用户上下文列表
        /// </summary>
        internal Hashtable UserContextList
        {
            get
            {
            ...
            get
            {
                try
                {
                    lock (uclslocker)
                    {
                        ...
                        {
                            return Instance.UserContextList[CurrentUserInfo.LoginName] as UserContext;
                        }
        ...
        public static string AuthUser(string uname, string pwd, bool passwordEncrypted)
        {
            ...
            if (!String.IsNullOrEmpty(sid))
            {
                ...
                lock (Instance.UserContextList.SyncRoot)
                {
