...
namespace PIC.Portal.Web
{
    ...
    public class ContextModule : IHttpModule
    {
        ...
        internal static void AcquireRequestIdentity()
        {
            ...
            else if(PortalService.RunningMode == PortalService.RunningModeEnum.PerformanceTesting)
            {
                HttpCookie cookie = HttpContext.Current.Request.Cookies.Get(PortalService.TEST_USER_NAME_KEY);

                if (cookie != null && !String.IsNullOrEmpty(cookie.Value))
                {
                    OrgUser orgUser = OrgUser.GetByLoginName(cookie.Value);
                    ...
                    if (orgUser != null)
                    {
                        ...
                        // 测试模式下，可用登录名获取用户信息
                        UserInfo ui = PortalService.Provider.GetUserInfo(orgUser.LoginName);
