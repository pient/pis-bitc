...
namespace PIC.Portal.Web.Services
{
    ...
    public sealed class UserSessionServer : IDisposable
    {
        ...
        private Hashtable _sessions = Hashtable.Synchronized(new Hashtable());  // Key, Value: object, UserSession
        ...
        {
            IList<UserSession> orgSession = new List<UserSession>();

            foreach (UserSession us in _sessions.Values)
            {
                if (us.LogonInfo.UserID == userID)
                {
                    orgSession.Add(us);
                }
            }
        ...
        {
            UserSession userSession = _sessions[sessionID] as UserSession;
        ...
        public void RefreshSession(string sessionID)
        {
            ...
            if (_sessions.ContainsKey(sessionID))
            {
                userSession = _sessions[sessionID] as UserSession;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public IList<UserInfo> GetOnlineUserList()
        {
            IList<UserInfo> userList = new List<UserInfo>();
            foreach (UserSession us in Sessions.Values)
            {
                userList.Add(us.LogonInfo);
            }
        ...
        /// <summary>
        /// 由用户登录名获取用户信息（只有性能测试时有效）,其他状态返回空值
        /// </summary>
        /// <param name="loginName"></param>
        /// <returns></returns>
        public UserInfo GetLogonInfoByLoginName(string loginName)
        {
            UserSession userSession = this.GetSessionByLoginName(loginName);
        ...
        {
            UserSession rtn_us = null;
            foreach (UserSession us in Sessions.Values)
            {
                if (us.LogonInfo.LoginName == loginName)
                {
                    rtn_us = us;
                    break;
                }
