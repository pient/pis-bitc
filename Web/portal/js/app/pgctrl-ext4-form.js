...
Ext.define('PIC.ExtFormPanel', {
    ...
    constructor: function (config) {
        ...
        if (config.autolayout) {
            ...
            if (config.frmbuttons) {
                $.each(config.frmbuttons, function () {
                    if (!this.margins) {
                        this.margins = "2 5 5 2";
                    }
                });

                config.frmbuttons.unshift({ xtype: 'container', width: config.labelWidth  || 0 });

                var btncontainer = { xtype: 'container', layout: 'hbox', align: 'right', items: config.frmbuttons };
                config.items.push(btncontainer);
            }
        }
